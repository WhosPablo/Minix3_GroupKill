 ...
 */
  ...
  for (rmp = &mproc[NR_PROCS-1]; rmp >= &mproc[0]; rmp--) {
	...
	if (mp->mp_effuid != SUPER_USER          /*If signal was not sent by SU*/
	    && mp->mp_realuid != rmp->mp_realuid /*and if signal was not sent by original user*/
	    && mp->mp_effuid != rmp->mp_realuid    
	    && mp->mp_realuid != rmp->mp_effuid
	    && mp->mp_effuid != rmp->mp_effuid
      && mp->mp_realgid != rmp->mp_realgid /*and if signal was not sent by user in same group*/
      && mp->mp_effgid != rmp->mp_realgid
      && mp->mp_realgid != rmp->mp_effgid
      && mp->mp_effgid != rmp->mp_effgid) {
		error_code = EPERM;                     /*Then end error*/
		continue;
	}
